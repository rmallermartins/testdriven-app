...
then
  ...
  if [[ "$TRAVIS_BRANCH" == "staging" ]]; then
    ...
    export REACT_APP_USERS_SERVICE_URL="http://testdriven-staging-alb-620745421.sa-east-1.elb.amazonaws.com"
  elif [[ "$TRAVIS_BRANCH" == "production" ]]; then
    export DOCKER_ENV=prod
    export REACT_APP_USERS_SERVICE_URL="http://testdriven-production-alb-1404726488.sa-east-1.elb.amazonaws.com"
  fi
  ...
  then
    ...
    export REPO=$AWS_ACCOUNT_ID.dkr.ecr.sa-east-1.amazonaws.com
  fi
